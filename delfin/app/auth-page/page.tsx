  ...
  return (
    <div className="flex flex-col items-center justify-center w-screen h-screen">
      <div className="flex flex-col p-4 relative items-center bg-slate-100 w-[500px] h-[300px] rounded-lg border-2 border-black">
        <h1 className="font-bold text-xl mb-10">{authParam ? "Login" : "Signup"}</h1>
        <form action={authParam ? handleClickLoginButton : handleClickSignupButton} className="flex flex-col gap-4">
          <input type="text" name="email" placeholder="Email" disabled={isPending} className="rounded-lg p-2 text-black" />
          <input type="password" name="password" placeholder="Password" disabled={isPending} className="rounded-lg p-2 text-black" onChange={(e) => setPassword(e.target.value)} />
          {!authParam && (
            <input
              type="password"
          ...
          <div className="flex justify-between">
            <button className="p-2 bg-blue-600 rounded-md text-white" disabled={isPending}>
              {authParam ? (isPending ? "Logging in..." : "Log in") : isPending ? "Signing un..." : "Sign up"}
            </button>
